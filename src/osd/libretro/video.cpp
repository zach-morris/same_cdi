...
bool retro_osd_interface::video_init()
{
	...
	for (index = 0; index < video_config.numscreens; index++)
	{
		...
		// create window ...
		std::shared_ptr<retro_window_info> win = std::make_shared<retro_window_info>(machine(), index, m_monitor_module->pick_monitor(reinterpret_cast<osd_options &>(options()), index), &conf);
...
void retro_osd_interface::extract_video_config()
{
	...
	video_config.scale_mode = 0;//drawretro_scale_mode(stemp);
