...
static const char cdimono1_cfg_default[] =
   ...
   "                </newseq>\n"
   "            </port>\n"
   "            <port tag=\":slave_hle:MOUSEBTN\" type=\"P1_BUTTON3\" mask=\"4\" defvalue=\"0\">\n"
   "                <newseq type=\"standard\">\n"
   "                    JOYCODE_1_BUTTON1\n"
   "                </newseq>\n"
   "            </port>\n"
...
bool configuration_manager::load_settings()
{
	...
	// TODO/FIXME: For the libretro SAME CDi core,
	// we need to generate a default game-specific
	// file if one does not already exist. This should
	// be done in the OSD platform code, but using
	// emu_file there (or any regular libretro
	// filestream code) prevents the core from building
	// (DWARF error: section .debug_str is larger than
	// its filesize). As a temporary workaround, we hack
	// in the default file creation here...
	if (filerr && (machine().basename() == "cdimono1"))
	{
		osd_printf_verbose("File not found - creating default: %s.cfg\n",machine().basename());
		file.set_openflags(OPEN_FLAG_WRITE | OPEN_FLAG_CREATE | OPEN_FLAG_CREATE_PATHS);
		filerr = file.open(machine().basename() + ".cfg");
		if (!filerr)
		{
			file.write(cdimono1_cfg_default, strlen(cdimono1_cfg_default));
			file.close();
		}
